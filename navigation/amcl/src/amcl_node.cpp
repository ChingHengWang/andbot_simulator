...
//scan_match
    if(max_weight > 0.0)
    {
       ...
       */
      double amcl_yaw, amcl_pitch, amcl_roll;
      tf::Matrix3x3(tf::Quaternion(p.pose.pose.orientation.x,p.pose.pose.orientation.y,p.pose.pose.orientation.z,p.pose.pose.orientation.w)).getEulerYPR(amcl_yaw,amcl_pitch,amcl_roll);

      if (m_file_pose_amcl != NULL)
      {
        fprintf(m_file_pose_amcl, "%.4f %.3f %.3f %.3f \n", p.header.stamp.toSec(),  p.pose.pose.position.x,  p.pose.pose.position.y, amcl_yaw);
      }
      else
      {
        printf("Can not Open Write pose to File  !!");
      }
